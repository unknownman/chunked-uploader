...
final class ChunkUploaderOrphanRollbackTest extends TestCase
{
    #[Test]
    public function test_a_stored_chunk_is_deleted_when_progress_cannot_be_recorded(): void
    {
        ...
        try {
            $manager->processChunk($this->chunk($this->temporaryFile('DATA'), 'token', 0, 1, 4, 'rollback'));
            self::fail('Expected the metadata failure to surface.');
    ...
    private function manager(ChunkStorageInterface $storage, MetadataRepositoryInterface $metadata): ChunkUploader
    {
